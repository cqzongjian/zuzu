...
enum VideoPagePositon {
  left,
  right,
  middle,
}

class VideoScaffoldController extends ValueNotifier<VideoPagePositon> {
  VideoScaffoldController([
    VideoPagePositon value = VideoPagePositon.middle,
  ]) : super(value);

  Future? animateToPage(VideoPagePositon pagePositon) {
    return _onAnimateToPage?.call(pagePositon);
  }

  Future? animateToLeft() {
    return _onAnimateToPage?.call(VideoPagePositon.left);
  }

  Future? animateToRight() {
    return _onAnimateToPage?.call(VideoPagePositon.right);
  }

  Future? animateToMiddle() {
    return _onAnimateToPage?.call(VideoPagePositon.middle);
  }

  Future Function(VideoPagePositon pagePositon)? _onAnimateToPage;
}
...
class _TikTokScaffoldState extends State<VideoScaffold>
  ...
  Future animateToPage(p) async {
    ...
    switch (p) {
      case VideoPagePositon.left:
        await animateTo(screenWidth!);
        break;
      case VideoPagePositon.middle:
        await animateTo();
        break;
      case VideoPagePositon.right:
        await animateTo(-screenWidth!);
        break;
    }
  ...
  onHorizontalDragEnd(details, screenWidth) {
    ...
    if (vOffset > scrollSpeed && inMiddle == 0) {
      // 去右边页面
      return animateToPage(VideoPagePositon.left);
    } else if (vOffset < -scrollSpeed && inMiddle == 0) {
      // 去左边页面
      return animateToPage(VideoPagePositon.right);
    } else if (inMiddle > 0 && vOffset < -scrollSpeed) {
      return animateToPage(VideoPagePositon.middle);
    } else if (inMiddle < 0 && vOffset > scrollSpeed) {
      return animateToPage(VideoPagePositon.middle);
    }
...
class _MiddlePage extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    Widget tabBarContainer = bottomNavigationBar ??
        Container(
          height: 44,
          child: Placeholder(
            color: Colors.red,
